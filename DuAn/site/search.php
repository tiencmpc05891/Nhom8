...
try {
    $pdo = new PDO("mysql:host=localhost;dbname=duanmau", "root", "mysql");
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
} catch (PDOException $e) {
    die("Không thể kết nối đến cơ sở dữ liệu: " . $e->getMessage());
}

// Kiểm tra xem có dữ liệu từ trường tìm kiếm không
if (isset($_GET['search_query'])) {
    $searchTerm = $_GET['search_query'];

    // Sử dụng truy vấn SQL để tìm kiếm sản phẩm dựa trên tên sản phẩm hoặc mô tả
    $query = "SELECT * FROM hanghoa WHERE TenHH LIKE :searchTerm OR GhiChu LIKE :searchTerm";
    $statement = $pdo->prepare($query);
    $searchParam = '%' . $searchTerm . '%';
    $statement->bindParam(':searchTerm', $searchParam);
    $statement->execute();
    ...
    if ($statement->rowCount() > 0) {
        ...
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
            ...
            // Hiển thị sản phẩm tương tự với mã HTML tương tự như bạn đã làm trong trang gốc
